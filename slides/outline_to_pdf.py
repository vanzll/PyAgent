"""Generate a formatted PDF of the PyCallingAgent talk speaker outline (23 slides)."""
...
outpath = "/mnt/data/wanzl/pycallingagent/slides/speaker_outline.pdf"
doc = SimpleDocTemplate(outpath, pagesize=A4, topMargin=20*mm, bottomMargin=20*mm, leftMargin=22*mm, rightMargin=22*mm, title="PyCallingAgent Talk Outline", author="PyCallingAgent Team")
...
s.append(Paragraph("PyCallingAgent 45-Minute Research Talk", sTitle))
...
s.append(Paragraph("代表工作：PyCallingAgent、EBC (ICML 2025)、POI Rec (AAAI 2025 oral)", sSB, bulletText=""))
...
s.append(Paragraph("PyCallingAgent 是 HKGAI 核心技术产品 HK E-Link 的底层框架", sSB, bulletText=""))
...
s.append(Paragraph("左右对比：当前 text-bound 方法 vs PyCallingAgent runtime-native", sB, bulletText="\u2022"))
...
s.append(Paragraph("PyCallingAgent.run() 核心循环", sB, bulletText="\u2022"))
...
s.append(Paragraph("对比表：Standard vs PyCallingAgent Skills（函数、状态、成本）", sB, bulletText="\u2022"))
...
s.append(Paragraph("11/12 设置 PyCallingAgent 胜出", sB, bulletText="\u2022"))
